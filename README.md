...
- 支持按账号 ID / Hive ID 模糊搜索
- 一键兑换全部账号，或只兑换指定账号
- 按小时定时检查 SWGT；发现未被启用账号成功兑换过的新码时才自动触发兑换和通知
- 在 Web 后台设置检查间隔、时区和随机延迟
...
该 GHCR 镜像已设置为公开，可匿名拉取，不需要执行 `docker login`。默认配置也不需要创建 `.env`。如需修改端口、定时检查间隔或其他选项，可在 `environment` 中加入下方配置项。
...
- 是否启用定时检查
- 检查间隔（`0.5–168` 小时，以半小时递增，默认每小时）
- 执行时间使用的 IANA 时区，默认 `Asia/Shanghai`
- 两次兑换码之间的随机延迟，默认 `4.5–12 秒`
- 检查账号到确认兑换之间的随机延迟，默认 `0.8–2.2 秒`

设置保存在 SQLite 中。定时器会动态读取最新设置；随机延迟从设定的最小值和最大值之间均匀抽取，并从下一次任务开始生效。

每次检查只请求 SWGT 的兑换码列表；没有发现至少一个已启用账号尚未成功兑换或确认兑换的新码时，不会启动 Chromium，也不会发送 Telegram 通知。兑换记录会永久去重，避免同一账号重复兑换同一礼包码。
...
2. 先给新机器人发送一条消息。
3. 通过 Bot API 获取目标对话的 Chat ID；群组 Chat ID 通常以 `-` 开头。
4. 登录控制台，进入“Telegram 推送”，填写 Bot Token 和 Chat ID。
5. 点击“发送测试消息”，确认收到后启用任务推送并保存。

启用后，手动任务结束时会推送结果；定时任务只会在发现符合条件的新码并启动兑换后推送，消息会包含兑换码、成功、跳过、失败数量以及致命错误。Bot Token 只保存在 SQLite 中，接口不会将原 Token 返回给浏览器。

Bot 命令通过 Telegram Webhook 接收，因此必须使用公网 HTTPS 地址访问后台并保存 Telegram 配置。仅配置的 Chat ID 可以执行命令：`/redeem` 立即兑换全部启用账号；`/records` 查询最近 10 条记录；`/records 20` 可查询最多 20 条；`/records 关键词` 可按兑换码、Hive ID 或账号备注搜索。
...
| `APP_TIMEZONE` | `Asia/Shanghai` | 通知时间使用的 IANA 时区 |
| `COUPON_CHECK_ENABLED` | `true` | 是否启用定时检查新兑换码 |
| `COUPON_CHECK_HOURS` | `1` | 新兑换码检查间隔，0.5–168 小时，以半小时递增 |
