...
- 可为每个账号设置兑换服务有效期：自然月、每年或永久，并按账号 ID / Hive ID 模糊搜索
- 一键兑换全部账号，或只兑换指定账号
...
每次检查只请求 SWGT 的兑换码列表；没有发现至少一个已启用且服务有效的账号尚未成功兑换或确认兑换的新码时，不会启动 Chromium，也不会发送 Telegram 通知。兑换记录仍会永久去重，避免同一账号重复兑换同一礼包码。

账号的兑换服务有效期可在添加或编辑账号时设置。月度服务到当月最后一天到期；年度服务到次年同日前一天到期；永久服务不会到期。编辑账号时勾选“从今天起续期”可按所选套餐重新计算到期日；到期账号会保留记录，但不会参与兑换。
...
| `COUPON_CHECK_HOURS` | `1` | 新兑换码检查间隔，1–168 小时 |
