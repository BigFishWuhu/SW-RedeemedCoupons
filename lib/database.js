...
class AppDatabase {
    ...
    saveTelegramConfig(input) {
        ...
        const next = {
            ...
            webhookUrl: input.webhookUrl === undefined ? current.webhookUrl : String(input.webhookUrl || '').trim().replace(/\/+$/, ''),
            webhookSecret: current.webhookSecret || crypto.randomBytes(24).toString('hex')
        };
        ...
        upsert.run('telegram_webhook_url', next.webhookUrl);
        upsert.run('telegram_webhook_secret', next.webhookSecret);
        return next;
    }
    ...
    listAccounts(enabledOnly = false, query = '') {
        ...
        return this.db.prepare(`
            SELECT id, name, hive_id AS hiveId, server, enabled,
                created_at AS createdAt, updated_at AS updatedAt
            FROM accounts ${clause} ORDER BY id DESC
        `).all(...params).map((row) => ({ ...row, enabled: Boolean(row.enabled) }));
    }
...
function finiteIntervalHours(value, fallback) {
    ...
    if (!Number.isFinite(number) || number < 0.5 || number > 168 || number * 2 % 1 !== 0) {
        throw new Error('兑换码检查间隔必须是 0.5–168 小时之间、以半小时递增的数值');
    }
    return number;
}
